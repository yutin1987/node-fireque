...
exports.pushToCompleted = function (uuid, cb) {
    this._connection.lpush(this._getPrefixforProtocol() + ':completed', uuid, cb);
}
...
exports.pushToFailed = function (uuid, cb) {
    this._connection.lpush(this._getPrefixforProtocol() + ':failed', uuid, cb);
}
...
exports.fetchFromProcessing = function (cb) {
    this._connection.lrange( this._getPrefixforProtocol() + ':processing', -1000, 1000, cb);
}
...
exports.delJob = function (uuid, cb) {
    var keys = [
        this._getPrefix() + ':job:' + uuid,
        this._getPrefix() + ':job:' + uuid + ':timeout'
    ];
    this._connection.del( keys, cb);
}
...
exports.cleanJob = function (uuid, cb) {
    var queue = [
        ...
        ':completed',
        ':failed',
        ':buffer:' + this.protectKey + ':high',
        ':buffer:' + this.protectKey + ':med',
        ':buffer:' + this.protectKey + ':low',
...
exports.getTimeoutOfJob = function (uuid, cb) {
    this._connection.ttl(this._getPrefix() + ':job:' + uuid + ':timeout', cb);
}
...
exports.fetchScheduleByTimestamp = function (timestamp, cb) {
    ...
    this._connection.keys( this._getPrefixforProtocol() + ':schedule:*', function (err, reply) {
        if ( err == null && reply ) {
            for (var i = 0, length = reply.length; i < length; i+=1) {
                time = (reply[i] && reply[i].split(':')[4]) || false;
                if ( time !== false && time <= timestamp ) {
                    keys.push(time);
                }
            };
        }
