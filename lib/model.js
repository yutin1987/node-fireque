...
exports.popFromCompleted = function (cb) {
    var prefix = this._getPrefixforProtocol();
    if ( util.isArray(prefix) ) {
        prefix = prefix.map(function(key){
            return key + ':completed';
        }.bind(this));
        prefix.push(1);
    }else{
        prefix = [prefix + ':completed', 1];
    }

    this._connection.brpop(prefix, function (err, reply) {
...
exports.fetchFromProcessing = function (cb) {
    ...
    async.map(prefix, function (item, cb) {
        this._connection.lrange( item, -1000, 1000, cb);
    }.bind(this), function (err, result) {
        cb && cb(err, (err === null) && Array.prototype.concat.apply([], result));
    });
}
